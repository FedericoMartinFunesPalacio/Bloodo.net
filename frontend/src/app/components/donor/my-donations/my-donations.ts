...
export class MyDonationsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.authService.getCurrentUser().pipe(takeUntil(this.destroy$)).subscribe(user => {
      if (user && user.roleId) {
        const newDonorId = Number(user.roleId);
        if (newDonorId !== this.donorId) {
          this.stats = null;
          this.health = null;
          this.subscribedCampaigns = [];
          this.animated = false;
          this.error = false;
        }
  ...
  private loadData(): void {
    ...
    this.donorService.getDonorHealth().subscribe({
      next: (health) => {
        this.health = health;
  ...
  private loadSubscribedCampaigns(): void {
    this.campaignService.getActiveSubscribedCampaigns().subscribe({
      next: (campaigns) => {
  ...
  confirmUnsubscribe(): void {
    ...
    this.campaignService.unsubscribeFromCampaign(this.confirmCampaignId).subscribe({
      next: () => {
