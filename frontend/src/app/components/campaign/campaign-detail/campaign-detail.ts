...
import { CampaignBloodEstimated, CampaignLivesSaved, BloodTypeRanking, DonorHealth } from '../../../models/metrics';
import { MatIcon } from '@angular/material/icon';
import {DonorService} from '../../../services/donor.service';
import { LoadingComponent } from '../../reusable/loading/loading';
import { SubscribeConfirmModalComponent } from '../../reusable/subscribe-confirm-modal/subscribe-confirm-modal';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, EnumLabelPipe, MatIcon, LoadingComponent, SubscribeConfirmModalComponent],
...
export class CampaignDetailComponent implements OnInit, OnDestroy {
  ...
  modalDonorHealth: DonorHealth | null = null;
  modalDonorGender: Gender | null = null;
  modalDonorWeight: number | null = null;
  ...
  suscribeEvent(campaingId: number): void {
    this.campaignService.subscribeDonor(campaingId).subscribe({
      next: () => {
  ...
  subscribeToCampaign(): void {
    ...
    this.authService.getCurrentUser().pipe(takeUntil(this.destroy$)).subscribe({
      next: (user) => {
        if (!user?.roleId) {
          this.showSubscribeConfirm = true;
          return;
        }

        forkJoin({
          health: this.donorService.getDonorHealth(),
          donor: this.donorService.getDonorById(user.roleId)
        }).pipe(takeUntil(this.destroy$)).subscribe({
          next: ({ health, donor }) => {
            this.modalDonorHealth = health;
            this.modalDonorGender = donor.gender;
            this.modalDonorWeight = donor.weight;
            this.showSubscribeConfirm = true;
          },
          error: () => {
            this.showSubscribeConfirm = true;
  ...
  onSubscribeConfirm(): void {
    this.showSubscribeConfirm = false;
        ...
        next: (user) => {
          ...
          this.donorService.getDonorById(user.roleId).subscribe({
            next: (donor) => {
              ...
              this.suscribeEvent(this.campaign!.id!);
            },
            error: (err) => {
  ...
  unsubscribeFromCampaign(): void {
    ...
    this.campaignService.unsubscribeDonor(this.campaign!.id!).subscribe({
      next: () => {
